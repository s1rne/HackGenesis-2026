...
#   rake profiles    — одна очередь, восемь наборов настроек
#   rake fuzz        — обстрел роутера случайными очередями
...
# На выданной очереди почти каждая заявка допустима у одного провайдера, и
# половина профилей даёт то же распределение, что базовый. Со стороны это
# неотличимо от «настройки не работают», поэтому есть очередь, где допустимы
# сразу трое: на ней видно, что профили действительно решают.
desc "Сравнение профилей на очереди, где есть из кого выбирать"
task :profiles do
  sh RUBY, "bin/route", "compare", "--queue", "test/fixtures/profile_demo_queue.json"
end

# Боевую очередь выдают за час до стопкода. Единственная страховка, доступная
# заранее, — прогнать конвейер на очередях, которых никто не писал руками.
desc "Обстрел роутера случайными очередями (CASES=200, SEED=1)"
task :fuzz do
  sh RUBY, "tools/fuzz.rb", "--cases", ENV.fetch("CASES", "200"), "--seed", ENV.fetch("SEED", "1")
end
