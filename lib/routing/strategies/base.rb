...
module Routing
  module Strategies
    ...
    class Base
      ...
      # Уже ли величина выражена в готовой шкале 0..1.
      #
      # По умолчанию нет: рубли недобора, номер приоритета и доля дефицита
      # несравнимы между собой, и привести их к общей шкале может только
      # нормализация по кандидатам. Но у неё есть цена — min-max растягивает
      # любую разницу до максимума: «лучше на волос» и «лучше вдвое» дают
      # одинаковые 1 и 0. Для величины, у которой 0..1 есть собственный
      # смысл, это искажение, и такая цель объявляет absolute? = true.
      def absolute? = false
